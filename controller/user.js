...
const createUser = async (req, res) => {
    try {
		...
		const saltRound = 10; // more salt, more secure
...
const getUser = async (req, res) => {
    try {
        ...
        const user = await User.findOne({ _id: id })
            .select(['email', 'bio', '_id', 'createdAt', 'updatedAt'])  // tengok variable table dekat mongodb // -password = dont send password
            .exec();
...
const login = async (req, res) => {
    try {
        ...
        const token = await jwt.sign({ email: user.email, id: user?._id }, jwtSecret); // add expired for security
...
const deleteUser = async (req, res) => {
    try {
        const id = req.id; // check for user is actually exist
